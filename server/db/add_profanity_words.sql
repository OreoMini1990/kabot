...
-- ============================================
-- SQLite용 (INSERT OR IGNORE 사용)
-- ============================================

/*
-- SQLite에서는 위의 SQL에서 ON CONFLICT (word) DO NOTHING을 INSERT OR IGNORE로 변경
...
-- ============================================
-- 
-- 1. Supabase (PostgreSQL) 사용 시:
--    - Supabase 대시보드의 SQL Editor에서 위의 SQL 실행
--    - 또는 psql 클라이언트에서 실행
--
-- 2. SQLite 사용 시:
--    - 주석 처리된 SQLite 버전 사용 (INSERT OR IGNORE)
--    - 또는 수동으로 ON CONFLICT -> INSERT OR IGNORE로 변경
--
-- 3. 중복 체크:
--    - ON CONFLICT (word) DO NOTHING (PostgreSQL)
