...
2. **`server/db/reports_schema.sql`** 실행
   - 신고 테이블 생성
   - 실행 시간: 약 30초
   - **필수**: 신고 기능 사용 시 필요

3. **`server/db/chat_logs_aggregation.sql`** 실행 (선택사항)
   - 통계 집계 함수 생성
   - 실행 시간: 약 30초
   - **선택**: 통계 집계 기능 사용 시 필요

4. **`server/db/chat_logs_search.sql`** 실행 (선택사항)
   - 검색 함수 생성
   - 실행 시간: 약 30초
