...
/** OAuth 연동 시작 URL (NAVER_OAUTH_BASE_URL 사용 시 Vercel 등 외부 인증 가능) */
const DEFAULT_NAVER_OAUTH_BASE = 'https://medifirstall.vercel.app';

function getNaverOAuthStartUrl(userIdStr, draftId = null, userName = null) {
    let base = process.env.NAVER_OAUTH_BASE_URL || '';
    if (!base || !base.trim()) {
        const serverUrl = process.env.SERVER_URL || `http://${process.env.SERVER_HOST || 'localhost'}:${process.env.PORT || 5002}`;
        const isLikelyLocal = /^(https?:\/\/)?(localhost|127\.0\.0\.1|192\.168\.|10\.|172\.(1[6-9]|2[0-9]|3[01])\.)/i.test(serverUrl);
        base = isLikelyLocal ? DEFAULT_NAVER_OAUTH_BASE : serverUrl;
    }
    base = String(base).replace(/\/$/, '');
    const p = (process.env.NAVER_OAUTH_START_PATH || '/oauth/naver/start').replace(/^\//, '');
    const url = `${base}/${p}`;
    const params = new URLSearchParams({ user_id: userIdStr });
    if (draftId) params.set('draft_id', draftId);
    if (userName && String(userName).trim()) params.set('user_name', String(userName).trim());
    const out = `${url}?${params.toString()}`;
    const baseSource = process.env.NAVER_OAUTH_BASE_URL ? 'env' : (base === DEFAULT_NAVER_OAUTH_BASE ? '기본(Vercel)' : 'SERVER_URL');
    console.log(`[질문 제출] OAuth 연동 URL base=${base} path=/${p} (base출처=${baseSource})`);
    return out;
}
...
async function processQuestionSubmission(room, sender, title, content, imageUrl = null, json = null) {
    ...
    // 토큰 존재 여부 확인 (user_id 기준 연동)
    const hasToken = await hasNaverToken(userIdStr);
    
    if (!hasToken) {
        // 토큰 없음: 연동 링크 제공 (NAVER_OAUTH_BASE_URL 사용 시 Vercel 등 외부 인증)
        ...
        const authUrl = getNaverOAuthStartUrl(userIdStr, draftId, senderName);
        replies.push(
            `🔗 네이버 계정 연동이 필요합니다.\n\n` +
    ...
    try {
        ...
        if (tokenResult.error) {
            if (tokenResult.error === 'token_not_found') {
                // 토큰이 없음 (레코스 삭제됨)
                const authUrl = getNaverOAuthStartUrl(userIdStr, null, senderName);
                replies.push(
                    `🔗 네이버 계정 연동이 필요합니다.\n\n` +
    ...
    try {
        ...
        if (result.success && result.articleUrl) {
            ...
            let replyMsg;
            const authorLine = senderName ? `작성자 : ${senderName}\n\n` : '';
            if (imageBuffers.length > 0) {
                // 이미지 포함 템플릿 (작성자 닉네임 제목 위)
