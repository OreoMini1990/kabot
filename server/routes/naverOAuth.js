 ...
 * @param {string} [draftId] - Draft ID (선택)
 * @param {string} [userName] - 채팅 닉네임 (연동 표시용, 선택)
 */
...
function verifyState(state) {
    try {
        ...
        // HMAC 검증용 payload 재구성 (draftId, userName 포함)
        const verifyPayload = { userId: payload.userId, nonce: payload.nonce, iat: payload.iat };
        if (payload.draftId) verifyPayload.draftId = payload.draftId;
        if (payload.userName) verifyPayload.userName = payload.userName;
...
router.get('/start', async (req, res) => {
    try {
        ...
        const userName = (req.query.user_name || '').trim() || null;
...
router.get('/callback', async (req, res) => {
    ...
    try {
        ...
        if (!statePayload) {
            ...
            try {
                ...
                if (parsed.userId) {
                    ...
                    console.warn(`[OAUTH-CB] [State 검증] ⚠️ 직접 파싱 성공: user_id=${userId}, draft_id=${draftId || 'null'}, user_name=${userName || 'null'}`);
                } else {
            ...
            userName = (statePayload.userName && String(statePayload.userName).trim()) || null;
            console.log(`[OAUTH-CB] [State 검증] ✅ 성공`);
            console.log(`[OAUTH-CB]   user_id: ${userId}`);
            console.log(`[OAUTH-CB]   draft_id: ${draftId || 'null'}`);
            if (userName) console.log(`[OAUTH-CB]   user_name: ${userName}`);
        }
        ...
        // DB에 저장 (insert - is_active=true로 새 토큰 저장, user_name 있으면 함께 저장)
        const insertRow = {
            user_id: userId,
        ...
        if (userName) insertRow.user_name = userName;
        
        console.log(`[OAuth] [저장] 새 토큰 저장 시작: user_id=${userId}${userName ? `, user_name=${userName}` : ''}`);
        ...
        if (dbError) {
            ...
            if (userName) upsertRow.user_name = userName;
