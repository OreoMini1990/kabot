...
try:
    ...
    print(f"[DB 쿼리] ⚠️⚠️⚠️ dict 변환 완료: ...")  # 이 로그가 출력되지 않음
except Exception as e:
    messages = raw_messages  # fallback

# client/a.py - poll_messages()
messages = get_new_messages()  # dict 리스트 또는 튜플 리스트
if messages:  # 이 조건이 false일 수 있음
    # 메시지 처리
    new_messages = []
    for msg in messages:
        # 필터링
    ...
    if new_messages:  # 이 조건이 false일 수 있음
        # 루프 진입
```

**원인**:
1. `get_new_messages()` 함수에서 dict 변환 중 예외 발생하여 fallback으로 튜플 리스트 사용
2. `poll_messages()` 함수에서 `if messages:` 조건이 false
3. `new_messages` 필터링 후 `if new_messages:` 조건이 false
...
// server/server.js
if (isReplyMessage) {
    if (!json?.attachment_decrypted && json?.attachment && ...) {
        try {
            const myUserId = json?.myUserId || json?.userId || null;
            const encType = json?.encType || null;
            
            if (myUserId && encType) {  // 이 조건이 false일 수 있음
                // 여러 후보로 복호화 시도
            }
        } catch (decryptError) {
            // 예외 처리
...
- `poll_messages()` 함수에서 `messages` 및 `new_messages` 상태 확인
- 예외 발생 시 상세 로그 출력

**코드 변경**:
- `[DB 쿼리] ⚠️⚠️⚠️ dict 변환 완료` 로그를 항상 출력하도록 수정
- `[디버그] ⚠️⚠️⚠️ new_messages 상태` 로그를 항상 출력하도록 수정
...
**코드 변경**:
- `server/db/utils/attachmentExtractor.js`에서 base64 디코딩 및 패턴 매칭 fallback 추가
